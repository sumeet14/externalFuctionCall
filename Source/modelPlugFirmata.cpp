...
class firmataBoardHandler  {
	firmataBoardHandler(){
		...
		printf("FirmataboardHandler constructor end\n"); fflush(stdout);
	}
...
double firmataBoard::readAnalogPin(uint32_t pin,double min, double max, double init){
	printf("%s", "readAnalogPin firmata class");
	fflush(stdout);
	if(ready){
...
/*__declspec (dllexport) __stdcall*/ void* boardConstructor(char* port,bool showCapabilitites,int samplingMs,int baudRate,bool dtr){
...
/*__declspec (dllexport) __stdcall*/ void boardDestructor(void* object){
	if(object!=NULL){
...
/*__declspec (dllexport) __stdcall*/ void updateBoard(int id){
	firmataBoard* board = firmataBoardHandler::instance()->getBoard(id);
	board->updateBoard(0);
}

/*__declspec (dllexport) __stdcall*/ int getBoardId(void* object){
	if(object!=NULL){
...
/*__declspec (dllexport) __stdcall*/ double readAnalogPin(int pin, double min, double max, double init, int id){
	printf("%s", "readAnalogPin external function");
	fflush(stdout);
	firmataBoard* board = firmataBoardHandler::instance()->getBoard(id);
	printf("%s", "readAnalogPin internal function called");fflush(stdout);
	double value = board->readAnalogPin(pin,min,max,init);
	printf("%s", "readAnalogPin internal function successfully called");fflush(stdout);
	return value;
}

/*__declspec (dllexport) __stdcall*/ int readDigitalPin(int pin, int init, int id){
	firmataBoard* board = firmataBoardHandler::instance()->getBoard(id);
	int value = board->readDigitalPin(pin,init);
	return value;
}

/*__declspec (dllexport) __stdcall*/ void writeAnalogPin(int pin, int id,double value){
	double croped = value>1.0?1.0:(value<0.0?0.0:value);
	firmataBoard* board = firmataBoardHandler::instance()->getBoard(id);
	board->writeAnalogPin(pin,(uint32_t)(croped*1023));
}

/*__declspec (dllexport) __stdcall*/ void writeDigitalPin(int pin, int id,int value){
	firmataBoard* board = firmataBoardHandler::instance()->getBoard(id);
	board->writeDigitalPin(pin,value);
}

/*__declspec (dllexport) __stdcall*/ void writeServoPin(int pin, int id,double value,int min,int max){
	double croped = value>1.0?1.0:(value<0.0?0.0:value);
	firmataBoard* board = firmataBoardHandler::instance()->getBoard(id);
